...
def test_phases_of_one_event_get_distinct_lines():
    ...
    lines = [
        _line(cog, _notif(1, "SvS", "borders_open")),
        _line(cog, _notif(2, "SvS", "teleport_window")),
        _line(cog, _notif(3, "SvS", "battle_start")),
    ]

    assert "SvS - Borders Open" in lines[0]
    assert "SvS - Teleport Window" in lines[1]
    assert "SvS - Battle Start" in lines[2]
...
def test_label_is_not_repeated_when_the_title_already_names_the_phase():
    ...
    cog.cursor.execute(
        ...
        " VALUES (1, 'SvS Borders Open')")
    cog.conn.commit()

    line = _line(cog, _notif(1, "SvS", "borders_open"))
