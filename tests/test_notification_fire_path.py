...
def _store_embed(cog, description):
    ...
    cog.cursor.execute(
        ...
        " VALUES (1, 'SvS Notification', ?)", (description,))
    cog.conn.commit()
...
def test_edited_embed_description_survives_the_send():
    ...
    asyncio.run(cog.process_notification(_embed_row("SvS", "borders_open")))

    sent = chan.sent[0]["embed"]
    default = event_types.EVENT_CONFIG["SvS"]["descriptions"]["borders_open"]
...
def test_empty_embed_description_falls_back_to_the_instance_default():
    ...
    sent = chan.sent[0]["embed"]
    default_start = event_types.EVENT_CONFIG["SvS"]["descriptions"]["borders_open"].split("%t")[0]
